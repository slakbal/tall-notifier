  <div x-transition:enter="transition transform-gpu ease-in" x-transition:enter-start="opacity-0 -translate-y-full"
      ...
      <div class="@if($message['closable']) cursor-pointer @endif flex flex-col select-none {{ $message['msgClass'] }} {{ config('tall-notifier.defaultMsgClass')}} overflow-hidden">
          <div class="flex flex-row">
              <div class="flex flex-col items-center justify-center p-2">
                  @if ($message['icon'])
                      {!! $message['icon'] !!}
                  @else
                       @include(config('tall-notifier.types.'.$message['type'].'.icon'))
                  @endif
              </div>
